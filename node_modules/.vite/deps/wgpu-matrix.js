//#region node_modules/wgpu-matrix/dist/3.x/wgpu-matrix.module.js
...
function getAPIImpl$5(Ctor) {
	...
	*/
	function truncate(a, maxLen, dst) {
		const newDst = dst ?? new Ctor(2);
		if (length(a) > maxLen) return setLength(a, maxLen, newDst);
	...
	*/
	function midpoint(a, b, dst) {
		return lerp(a, b, .5, dst ?? new Ctor(2));
	}
	return {
		create,
		fromValues,
...
function getAPIImpl$4(Ctor) {
	...
	*/
	function angle(a, b) {
		...
		const by = b[1];
		const bz = b[2];
		const mag = Math.sqrt(ax * ax + ay * ay + az * az) * Math.sqrt(bx * bx + by * by + bz * bz);
		const cosine = mag && dot(a, b) / mag;
		return Math.acos(cosine);
	}
	/**
	...
	*/
	function transformMat4Upper3x3(v, m, dst) {
		...
		const v1 = v[1];
		const v2 = v[2];
		newDst[0] = v0 * m[0] + v1 * m[4] + v2 * m[8];
		newDst[1] = v0 * m[1] + v1 * m[5] + v2 * m[9];
		newDst[2] = v0 * m[2] + v1 * m[6] + v2 * m[10];
		return newDst;
	}
	/**
	...
	*/
	function midpoint(a, b, dst) {
		return lerp(a, b, .5, dst ?? new Ctor(3));
	}
	return {
		create,
		fromValues,
...
function getAPIImpl$2(Ctor) {
	...
	*/
	function perspective(fieldOfViewYInRadians, aspect, zNear, zFar, dst) {
		const newDst = dst ?? new Ctor(16);
		const f = Math.tan(Math.PI * .5 - .5 * fieldOfViewYInRadians);
		newDst[0] = f / aspect;
		newDst[1] = 0;
		newDst[2] = 0;
		newDst[3] = 0;
	...
	*/ function perspectiveReverseZ(fieldOfViewYInRadians, aspect, zNear, zFar = Infinity, dst) {
		const newDst = dst ?? new Ctor(16);
		const f = 1 / Math.tan(fieldOfViewYInRadians * .5);
		newDst[0] = f / aspect;
		newDst[1] = 0;
		newDst[2] = 0;
		newDst[3] = 0;
...
function getAPIImpl$1(Ctor) {
	...
	**/
	function fromAxisAngle(axis, angleInRadians, dst) {
		const newDst = dst ?? new Ctor(4);
		const halfAngle = angleInRadians * .5;
	...
	*/
	function fromEuler(xAngleInRadians, yAngleInRadians, zAngleInRadians, order, dst) {
		const newDst = dst ?? new Ctor(4);
		const xHalfAngle = xAngleInRadians * .5;
		const yHalfAngle = yAngleInRadians * .5;
		const zHalfAngle = zAngleInRadians * .5;
		...
		switch (order) {
			...
			case "zyx":
				...
				newDst[3] = cx * cy * cz + sx * sy * sz;
				break;
			default: throw new Error(`Unknown rotation order: ${order}`);
		}
		return newDst;
	}
	/**
	...
	*/
	function rotationTo(aUnit, bUnit, dst) {
		...
		if (dot < -.999999) {
			...
			if (vec3.len(tempVec3) < 1e-6) vec3.cross(yUnitVec3, aUnit, tempVec3);
...
function getAPIImpl(Ctor) {
	...
	*/
	function midpoint(a, b, dst) {
		return lerp(a, b, .5, dst ?? new Ctor(4));
	}
	return {
		create,
		fromValues,
...
var { mat3, mat4, quat, vec2, vec3, vec4 } = wgpuMatrixAPI(Float32Array, Float32Array, Float32Array, Float32Array, Float32Array, Float32Array);
var { mat3: mat3d, mat4: mat4d, quat: quatd, vec2: vec2d, vec3: vec3d, vec4: vec4d } = wgpuMatrixAPI(Float64Array, Float64Array, Float64Array, Float64Array, Float64Array, Float64Array);
var { mat3: mat3n, mat4: mat4n, quat: quatn, vec2: vec2n, vec3: vec3n, vec4: vec4n } = wgpuMatrixAPI(ZeroArray, Array, Array, Array, Array, Array);
//#endregion
